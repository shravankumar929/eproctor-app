...
public class downloadPaper extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultSet rs = s.executeQuery("select * from Test where MacId=\"" + mac + "\" and ExamId=\""+exam_id+"\"");
        if (rs.next()) {
            try {
                ...
                int len=0;
                BufferedInputStream filein=new BufferedInputStream(new FileInputStream(new File("exam/"+exam_name+".json.txt")));
                response.setHeader("Content-Transfer-Encoding", "binary");
                
                fis = new FileInputStream(new File("exam/"+exam_name+".json.txt"));
                int i;
                char c;
                ...
                //set exam_start=1 when user downloads question paper
