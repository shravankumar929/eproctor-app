...
<html>
    ...
    <%
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("insert into Exam(Name,Time,Marks,Duration,NoOfQuestions,Description)values(?,?,?,?,?,?)");
                        ...
                        if (rs.next()) {
                            ...
                            if (rs.next()) {
                                ...
                                for (int opt = 0; opt < p.size(); opt++) {
                                    sql = "insert Options(QuestionId,Position,Option)values(?,?,?)";
                                    ps = con.prepareStatement(sql);
                                    ps.setInt(1, que_id);
...
String[] textareas= request.getParameterValues("textarea");
   ...
   for (int j = 0; j < checkboxes.length; j++) {
        ...
        out.println("<br>" + jsonString);
        try {
            String AnsPaperName = test_name + "_Ans" + ".json";
